...
class CLS_CATALOGS{
	...
	function Add_new(){
		$sql=" INSERT INTO `tbl_catalog`(`par_id`,`name`,`intro`,`isactive`) VALUES";
		$sql.="('".$this->ParID."','".$this->Name."','".$this->Intro."','".$this->isActive."')";
        //echo $sql;die();
		return $this->objmysql->Exec($sql);
	}
	function Update(){
		$sql = "UPDATE tbl_catalog SET par_id='".$this->ParID."',`name`=N'".$this->Name."',`intro`=N'".$this->Intro."',`isactive`='".$this->pro["isActive"]."' WHERE cat_id='".$this->ID."'";
		//echo $sql;die();
        return $this->objmysql->Exec($sql);
	}
